...
for l, p in zip(labels, probs):
    frac_o = np.array([compute_fraction(Po[idx]*p[idx]) for idx in keys]).T
    frac_i = np.array([compute_fraction(Pi[idx]*p[idx]) for idx in keys]).T
    frac_n = np.array([compute_fraction(Pn[idx]*p[idx]) for idx in keys]).T
